...
"""ID → :class:`ContainerInfo` lookup backed by ``podman inspect``.

Cached, bounded-timeout, soft-failing translator between a short
container id (all a hub signal carries) and the container's name +
OCI annotations.  Used by :class:`IdentityResolver` to label
notification bodies; available to any other client that needs the
same facts.

Previously lived in terok-sandbox; moved here because clearance is
the only consumer and carrying unused re-exports on sandbox's public
surface was confusing.
"""
